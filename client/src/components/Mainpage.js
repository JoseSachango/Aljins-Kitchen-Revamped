...
const Mainpage = () => {
  ...
  function pantryList() {
    if (isAuthenticated) {
      axios
        // would be changed to ingredients
        .get("/api/pantry/" + user.sub)
        .then((result) => {
          console.log(
            "This is the result from the get request to the endpoint with the usersId:",
            result.data
          );
        ...
        .catch((err) => {
          console.log(
            "There was an error with the get request to the endpoint with the userId: ",
            err
          );
        });
    }
  ...
  const handleSubmit = (event) => {
    ...
    setList([...list, search]); //Does this syntax mean you're pushing inputRef.current.value onto an array? How is that differnt than merging?Is this a controlled variable
    ...
    axios
      ...
      .then((result) => {
        if (result.data.userId) {
          console.log(
            "This is the value of user.sub before it is sent via put request: ",
            user.sub
          );
          console.log(
            "This is the value of list before it is sent via the put request: ",
            list
          );
          axios
            .put("/api/pantry/" + user.sub, { ingredients: list })
            .then((results) => {
              console.log(
                "The put request was successfully made. We returned the value: ",
                results
              );
            })
            .catch((err) => {
      ...
      .catch((err) => {
        console.log(
          "There was an error with the get request using the userId: ",
          err
        );
        console.log(
          "This is the value of user.sub when the get request return an error: ",
          user.sub
        );
        console.log(
          "This is the value of list when the get request returns an error: ",
          list
        );
        axios
  ...
  const getAuto = (e) => {
    ...
    const inputChars = e.target.value;
          ...
          <Paper className={classes.paper}>
            ...
            >
              ...
              <Autocomplete
                ...
                onChange={(e, value) => setSearch(value.food_name)}
                onInputChange={getAuto}
                renderInput={(params) => {
                  console.log(params);
                  return (
                    <TextField
